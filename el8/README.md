# RHEL 8 and compatibles
###### (currently using Oracle Linux 8)
...
`docker build -t python-altinstall:el8 .`

Then, to build the latest version:

`docker run --rm --volume "$PWD"/releases:/root/rpmbuild/SRPMS --volume "$PWD"/releases:/root/rpmbuild/RPMS python-altinstall:el8 python312-altinstall.spec`
